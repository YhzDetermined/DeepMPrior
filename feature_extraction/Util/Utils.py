...
from .FileHandler import write_activ_dict_to_csv
from .dataConvert import process_and_convert_activations, convert_activations
from ..entity.ActivInfo import ActivInfo
...
import numpy as np
import keras
from feature_extraction.Util.TimeCounter import TimeHistory
from keras.models import load_model
from ..callback.LossHistory import LossHistory
from  feature_extraction.Util.Logger import Logger

logger = Logger()
from keras.callbacks import ModelCheckpoint
import pickle
import time
import feature_extraction.Util.Module as md
...
def my_model_train(model,
    ...
    callbacks.append(loss_history)  # issue in lstm network

    callbacks_new = list(set(callbacks))
    if isinstance(dataset['x'], list):
        # model.build(input_shape=len(dataset['x']))
        model.build(input_shape = (None, len(dataset['x'][0])))
    else:
        model.build(input_shape=dataset['x'].shape)
    # fit model
    history = model.fit(dataset['x'], dataset['y'], batch_size=batch_size, validation_data=(
        dataset['x_val'], dataset['y_val']), epochs=iters, verbose=verb, callbacks=callbacks_new)
    check_point_model(model, checkpoint_dir, checkpoint_path, dataset, history)
    ...
    act_dic=loss_history.activate_dict
    layer_idx=loss_history.layer_idx
    layer_outputs = [model.layers[i].output for i in act_dic.keys()]
    # 创建一个新模型，输入为原模型的输入，输出为各层的输出
    activation_model = Model(inputs=model.input, outputs=layer_outputs)
    # 在测试数据上预测，获取激活值
    # with tf.device('/CPU:0'):
        # activations = activation_model.predict(dataset['x_val'],batch_size=batch_size)
    activations = activation_model.predict(dataset['x_val'],batch_size=batch_size)
    activations = convert_activations(activations)
    actInfo=ActivInfo(activations,layer_idx)
    actInfo.cal_dict()
    activ_dict=actInfo.get_dict()
    write_activ_dict_to_csv(save_dir,activ_dict)

    # 遍历各层，打印激活值
    # for i, activation in enumerate(activations):
    #     print(f"第 {i + 1} 层（{model.layers[i].name}）的激活值：")
    #     print(activation)
    return result, model
