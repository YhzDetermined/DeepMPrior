...
from get_mutation_info import Mutation_info
from Utils.utils import get_dense_weights
...
class MutationGenerator:
    def __init__(self, filename):
        ...
        self._model.summary()
        self.selected_mut_dic={}
        last_index = filename.rfind('/')
        self.base_dir = filename[:last_index + 1]
        ...
        # self.m_info = Mutation_info(self._model.get_weights(), self.base_dir)
        self.m_info = Mutation_info(get_dense_weights(self._model), self.base_dir)
        self.m_info.set_index_dict(self._model)
        model_name=int(os.path.basename(os.path.normpath(self.base_dir)))
        # and the list goes on and on...

    def _submit_model(self, v_path):
        self._submit_model_file(v_path,self._model)
    ...
    def apply_math_weight(self):
        for layer_no in range(0, len(self._model.layers)):
            ...
            for neuron_no in range(0, layer.units):
                ...
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_WEIGHT_1')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_WEIGHT_1')
                self._submit_model(v_path)
                v_path.pop()
                v_path.append('MATH_WEIGHT_2')
                weights[0][:, neuron_no] -= 2.0
                layer.set_weights(weights)
                # self.m_info.store_mutation_info(layer_no,neuron_no,self._model.get_weights(),'MATH_WEIGHT_2')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_WEIGHT_2')
                self._submit_model(v_path)
                weights[0][:, neuron_no] += 1.0
                ...
                weights[0][:, neuron_no] *= 2.0
                layer.set_weights(weights)
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_WEIGHT_3')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_WEIGHT_3')
                self._submit_model(v_path)
                v_path.pop()
                v_path.append('MATH_WEIGHT_4')
                weights[0][:, neuron_no] /= 4.0
                layer.set_weights(weights)
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_WEIGHT_4')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_WEIGHT_4')
                self._submit_model(v_path)
                end_time=time.time()
                weights[0][:, neuron_no] *= 2.0
                layer.set_weights(weights)  # restore the model
    ...
    def apply_math_bias(self):
        ...
        for layer_no in range(0, len(self._model.layers)):
            ...
            for neuron_no in range(0, layer.units):
                ...
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_BIAS_1')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_BIAS_1')
                self._submit_model(v_path)
                v_path.pop()
                v_path.append('MATH_BIAS_2')
                biases[bias_index][neuron_no] = biases[bias_index][neuron_no] - 2.0
                layer.set_weights(biases)
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_BIAS_2')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_BIAS_2')
                self._submit_model(v_path)
                biases[bias_index][neuron_no] = biases[bias_index][neuron_no] + 1.0
                ...
                biases[bias_index][neuron_no] = biases[bias_index][neuron_no] * 2.0
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_BIAS_3')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_BIAS_3')
                layer.set_weights(biases)
                self._submit_model(v_path)
                v_path.pop()
                v_path.append('MATH_BIAS_4')
                biases[bias_index][neuron_no] = biases[bias_index][neuron_no] / 4.0
                # self.m_info.store_mutation_info(layer_no, neuron_no, self._model.get_weights(), 'MATH_BIAS_4')
                self.m_info.store_mutation_info(layer_no, neuron_no, get_dense_weights(self._model), 'MATH_BIAS_4')
                layer.set_weights(biases)
                self._submit_model(v_path)
                biases[bias_index][neuron_no] = biases[bias_index][neuron_no] * 2.0
                layer.set_weights(biases)  # restore the model
    ...
    def _copy_model(self, ignored_layer_index):
        if ignored_layer_index >= len(self._model.layers) or not isinstance(self._model.layers[ignored_layer_index],
                                                                            Dense):
            return None
        ...
        for layer_index in range(0, len(self._model.layers)):
            print(f"The number of layer is:{layer_index}")
            if layer_index != ignored_layer_index:
    ...
    def apply_dup_layer(self):
        for layer_index in range(0, len(self._model.layers)-1):
    ...
    '''
    def store_dict(self):
        self.m_info.store_file()

    def get_dict(self):
        return self.m_info.get_mut_feat_dict()
